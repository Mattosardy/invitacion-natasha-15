...
const open_invitation_btn = document.getElementById("openInvitationBtn");
...
function verificar_fecha_limite() {
    ...
    if (ahora > fecha_limite) {
        ...
        mostrar_mensaje("El plazo de confirmacion finalizo el 29 de mayo.", "error");
        return true;
    }
    return false;
}
...
async function cargar_invitado() {
    ...
    try {
        ...
        mostrar_mensaje("Hubo un error al conectar con la base de datos.", "error");
        confirm_btn.disabled = true;
    }
}

function mostrar_acceso_invalido() {
    guest_name_display.textContent = "Acceso no valido";
    guest_name_display.style.color = "#ffd7df";
    confirm_btn.disabled = true;
    if (acceso_banner) acceso_banner.style.display = "block";
    mostrar_mensaje("Acceso no valido. Usa el enlace que recibiste por WhatsApp.", "error");
}

async function confirmar_asistencia() {
    if (verificar_fecha_limite()) return;
    if (!invitado_actual || !invitado_actual.token) {
        mostrar_mensaje("No se pudo identificar tu invitacion.", "error");
        return;
    }
    ...
    try {
        ...
        if (!data.ok) {
            confirm_btn.disabled = false;
            mostrar_mensaje("No se pudo registrar la confirmacion.", "error");
            return;
        }

        mostrar_mensaje("Gracias. Te espero para compartir una noche inolvidable.", "success");
    } catch (error) {
...
function controlar_musica() {
    if (musica_activa) {
        musica.pause();
        boton_musica.textContent = "Activar musica";
        musica_activa = false;
    } else {
        musica.play().then(() => {
            boton_musica.textContent = "Silenciar musica";
            musica_activa = true;
        }).catch(() => {
            mostrar_mensaje("Toca el boton para activar la musica.", "error");
        });
    }
}
...
if (open_invitation_btn) open_invitation_btn.addEventListener("click", abrir_invitacion);
...
console.log("Invitacion lista - confirmacion real con apps script");
